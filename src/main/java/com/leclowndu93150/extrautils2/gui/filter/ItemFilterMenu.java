...
public class ItemFilterMenu extends XUBaseMenu {
    public static final int GUI_W = 170;
    public static final int GUI_H = 243;
    ...
    public static final int BUTTON_ROW_1_Y = 108;
    public static final int BUTTON_ROW_2_Y = 128;
    private static final int PLAYER_INV_SLOT_X = 4;
    private static final int PLAYER_INV_SLOT_Y = 148;

    private final InteractionHand hand;
    private final Inventory playerInventory;
    private final ItemStackHandler ghosts = new ItemStackHandler(ItemFilterItems.NUM_SLOTS) {
        @Override
        protected void onContentsChanged(int slot) {
            ItemFilterItems.setGhostStack(getHeldStack(), slot, getStackInSlot(slot));
        }
    };

    public ItemFilterMenu(int id, Inventory playerInventory, InteractionHand hand) {
        super(ModMenus.ITEM_FILTER.get(), id);
        this.hand = hand;
        this.playerInventory = playerInventory;

        ItemStack held = getHeldStack();
        for (int i = 0; i < ItemFilterItems.NUM_SLOTS; i++) {
            ghosts.setStackInSlot(i, ItemFilterItems.getGhostStack(held, i));
        }
    ...
    public static ItemFilterMenu fromNetwork(int id, Inventory playerInventory, FriendlyByteBuf buf) {
        return new ItemFilterMenu(id, playerInventory, buf.readEnum(InteractionHand.class));
    }
    ...
    public Component getButtonLabel(int id) {
        ItemFilterItems.Flag flag = ItemFilterItems.Flag.values()[id - 1];
        return Component.translatable(isFlagEnabled(flag) ? flag.onKey : flag.offKey);
    }

    public boolean isFlagEnabled(ItemFilterItems.Flag flag) {
        return ItemFilterItems.getFlag(getHeldStack(), flag);
    }

    @Override
    public boolean clickMenuButton(Player player, int id) {
        if (id >= 1 && id <= ItemFilterItems.Flag.values().length) {
            ItemFilterItems.Flag flag = ItemFilterItems.Flag.values()[id - 1];
            ItemFilterItems.setFlag(getHeldStack(), flag, !isFlagEnabled(flag));
            return true;
        }
    ...
    public void clicked(int slotId, int button, ClickType clickType, Player player) {
        if (slotId >= 0 && slotId < ItemFilterItems.NUM_SLOTS) {
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        if (index < ItemFilterItems.NUM_SLOTS) {
    ...
    public boolean stillValid(Player player) {
        ItemStack held = getHeldStack();
        return !held.isEmpty() && held.is(ModItems.FILTER_ITEM.get());
    }
}
