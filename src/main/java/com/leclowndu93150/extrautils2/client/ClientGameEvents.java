...
public class ClientGameEvents {
    ...
    public static void renderOutline(RenderHighlightEvent.Block event) {
        ...
        if (event.getCamera().getEntity() instanceof LivingEntity living) {
            ...
            if (targetBlock.getBlock() instanceof DecorativeGlassBlock.Dark
                    || targetBlock.getBlock() instanceof DecorativeGlassBlock.IneffableDark) {
                Vec3 renderView = event.getCamera().getPosition();
                ((LevelRendererAccessor) event.getLevelRenderer()).invokeRenderHitOutline(
                        event.getPoseStack(),
                        event.getMultiBufferSource().getBuffer(XURenderTypes.LINES_NONTRANSLUCENT),
                        living,
